...
class build_resunetplusplus(nn.Module):
    def __init__(self, filter = 16):
        ...
        self.c1 = Stem_Block(3, filter, stride = 1)
        self.c2 = ResNet_Block(filter, filter * 2, stride = 2)
        self.c3 = ResNet_Block(filter * 2, filter * 4, stride = 2)
        self.c4 = ResNet_Block(filter * 4, filter * 8, stride = 2)

        self.b1 = ASPP(filter * 8, filter * 16)

        self.d1 = Decoder_Block([filter * 4, filter * 16], filter * 8)
        self.d2 = Decoder_Block([filter * 2, filter * 8], filter * 4)
        self.d3 = Decoder_Block([filter, filter * 4], filter * 2)

        self.aspp = ASPP(filter * 2, filter)
        self.output = nn.Conv2d(filter, 1, kernel_size = 1, padding = 0)
